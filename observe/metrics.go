...
var (
	labelProduce    = map[string]string{"client": "gokafka", "operation": "produce"}
	labelConsume    = map[string]string{"client": "gokafka", "operation": "consume"}
	labelProduceErr = map[string]string{"client": "gokafka", "operation": "produce"}
	labelConsumeErr = map[string]string{"client": "gokafka", "operation": "consume"}
)
...
func (c *Collector) OnProduce(nBytes int, err error) {
	...
	if err != nil {
		...
		c.emit(func(h MetricsRecorder) {
			h.IncCounter(c.name("produce_errors_total"), 1, labelProduceErr)
		})
		return
	}
	c.produced.Add(1)
	...
	c.emit(func(h MetricsRecorder) {
		h.IncCounter(c.name("produce_records_total"), 1, labelProduce)
		h.IncCounter(c.name("produce_bytes_total"), int64(nBytes), labelProduce)
	})
}
...
func (c *Collector) OnConsume(nBytes int, err error) {
	...
	if err != nil {
		...
		c.emit(func(h MetricsRecorder) {
			h.IncCounter(c.name("consume_errors_total"), 1, labelConsumeErr)
		})
		return
	}
	c.consumed.Add(1)
	...
	c.emit(func(h MetricsRecorder) {
		h.IncCounter(c.name("consume_records_total"), 1, labelConsume)
		h.IncCounter(c.name("consume_bytes_total"), int64(nBytes), labelConsume)
	})
}
...
func (c *Collector) OnRequest(apiKey int16, d time.Duration, err error) {
	...
	sec := d.Seconds()
	c.emit(func(h MetricsRecorder) {
		h.IncCounter(c.name("broker_requests_total"), 1, labels)
		h.RecordHistogram(c.name("broker_request_duration_seconds"), sec, labels)
	})
}
